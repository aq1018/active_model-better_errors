...
describe Emulation, '#add_on_blank' do
  subject { object }
  ...
  context 'when attributes is a single attribute' do
    ...
    before do
      object.add_on_blank(:first_name, options)
    end

    context 'when value responds to blank' do
      context 'and value is blank' do
        let(:first_name) { '' }

        its(:message) { should eql message }
        its(:type) { should eql :blank }
      end

      context 'and value is not blank' do
    ...
    context 'when value does not respond to blank' do
  ...
  context 'when attributes is an array' do
    ...
    before do
      object.add_on_blank([:first_name, :last_name], options)
    end
  ...
  context 'with default options' do
    ...
    before do
      object.add_on_blank(:first_name)
    end
